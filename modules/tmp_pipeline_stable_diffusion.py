...
class StableDiffusionPipeline(
    ...
    def __call__(
        ...
        **kwargs,
        ...
        step_size = 300 * np.sqrt(scale_range)
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                if apply_uc and t in timesteps_to_analyze:
                    ...
                    target_loss = 1e-6#1.9277e-05 #0.0001
                    iteration = 0
                    
                    aux_loss = torch.nn.functional.mse_loss(noise_pred_text, noise_pred_uncond,reduction = "none")
                    print(aux_loss.shape)

                    #print(aux_loss[1].mean().item(),aux_loss[2].mean().item(), aux_loss[3].mean().item())


                    do_it = True
                  
                    curr_aux = aux_loss[1].mean().item()
                    while curr_aux > target_loss and do_it and i > 5:
                        ...
                        noise_pred, intermediate_reps = noise_pred[0], noise_pred[1]
                        noise_pred_uncond, noise_pred_text = noise_pred.chunk(2)
                        aux_loss = torch.nn.functional.mse_loss(noise_pred_text, noise_pred_uncond,reduction = "none")[2]
                        

                        max_per_patch = aux_loss.max(dim=0).values  # [64,64]
                        threshold = max_per_patch.quantile(0.95)
                        mask = (max_per_patch >= threshold).float()  # [64,64]
                        aux_loss = aux_loss * mask  # gradients only flow for high-max patches
                        
                        aux_loss = aux_loss.mean()

                        #NORMAL
                        print(latent_model_input_grad.shape)
                        grads = torch.autograd.grad(
                            aux_loss,
                            latent_model_input_grad,
                            retain_graph=True,
                        ...
                        max_per_patch = grads.max(dim=0).values  # [64,64]
                        threshold = max_per_patch.quantile(0.95)
                        mask = (max_per_patch >= threshold).float()  # [64,64]
                        grads = (grads * mask).max()  # gradients only flow for high-max patches
